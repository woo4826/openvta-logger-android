...
import dev.openvta.logger.BuildConfig
import dev.openvta.logger.domain.FileNames
...
import dev.openvta.logger.domain.VtaFormatter
import dev.openvta.logger.domain.ZipFiles
...
class RecordingRepository(context: Context) {
    ...
    private fun writeMeta(session: RecordingSession) {
        ...
        File(sessionsDir, "${session.id}.meta").outputStream().use {
            properties.store(it, "OpenVTA Logger session metadata")
        }
    }
